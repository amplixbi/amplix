...
IG$.cVis.wordcloud.prototype.drawWordcloud = function() {
	...
	{
		colorset =  IG$.__chartoption.chartcolors[c_cset];
	}
    ...
    if (d.text.length > 20)
    {
      ...
      d.size = Math.abs(d.size);
			...
			{
				vmin = Math.min(vmin, d.size);
				vmax = Math.max(vmax, d.size);
			}
		}
		fdata.push(d);
	}
	...
	*/
	...
	d3.rollups(words, 
		...
		.slice(0, 250).map(function (_ref5) {
	...
	*/
	...
	function draw(words) {
		...
		var fill = function(t) {
			...
			if (colorset && colorset.length)
			{
				r = colorset[(cindex++) % colorset.length];
				colormap[t] = r;
			}
